...
int main(void) {
	...
	NX410_SSEG_setAllDigits(SSEGHI, CC_BLANK, CC_BLANK, CC_BLANK, CC_BLANK,
			DP_NONE);
	NX410_SSEG_setAllDigits(SSEGLO, CC_BLANK, CC_BLANK, CC_BLANK, CC_BLANK,
			DP_NONE);
	...
	while (1) { //While loop has a minimum period of 1 miliscond but is likely actually more because of calls that take long amounts of time
		...
		if (NX4IO_isPressed(BTNU)) { // If the Center Button is not pressed LightFollow uses the routine where it updates PWMs based on global variables that the FIT Handler updates from XADC signals
				...
				PWM_Set_Duty(PWM_BASEADDR,
						(int) (PWM_PERIOD
								* (DEFAULT_DUTY_PERCENTAGE
										+ vertical_Duty_Percentage_Modifier)
								/ 100.0f), 0); // DEFAULT_DUTY_PERCENTAGE currently causes a Duty Cycle of 4 milliseconds
				PWM_Set_Duty(PWM_BASEADDR,
						(int) (PWM_PERIOD
								* (DEFAULT_DUTY_PERCENTAGE
			...
			if (loopTracker == (3 * (MAIN_LOOP_TRACKER_MAX / 4))) { // Frequency of less than 10 Hz
				...
				servomotorAdjustmentCalculation(bufferUP, bufferDOWN, bufferLEFT,
						bufferRIGHT);
			}
		}
...
bool read_ADC(volatile uint16_t *targetDataPointer) {
	u8 ReadyBool = 0x00;
	...
	for (i = 0; i < 100; i++) {
		...
		if ((ReadyBool & 0x01) == 0x01) {
			digital_Data = Xil_In8(XADC_READ_GPIO_BASEADDR + GPIO2_IO); // 0x0008 should have been switch out for GPIO2_IO
